                ...
                sorted.map((r, i) => {
                  ...
                  return (
                    <tr key={i} className="border-t border-gray-50 hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-3 py-2.5 text-right">
  {inp.livrees && profit !== null ? (
    <span className={`text-xs font-black px-2 py-1 rounded-lg ${parseFloat(inp.livrees) * profit >= 0 ? "bg-green-50 text-green-600" : "bg-red-50 text-red-500"}`}>
      {(parseFloat(inp.livrees) * profit).toLocaleString()} DZD
    </span>
  ) : <span className="text-xs text-gray-300">—</span>}
</td>
                    </tr>
            ...
            <tfoot>
              <tr className="bg-gray-50 border-t-2 border-gray-200">
                <td colSpan={5} className="px-4 py-3 text-xs font-bold text-gray-500 uppercase">Total</td>
                ...
                <td className="px-3 py-3 text-right text-sm font-black text-green-600">
                  {sorted.reduce((s, r) => {
                    const profit = calcProfitPerUnit(r);
                    const inp = getInput(r.campaignName);
                    return s + (inp.livrees && profit !== null ? parseFloat(inp.livrees) * profit : 0);
