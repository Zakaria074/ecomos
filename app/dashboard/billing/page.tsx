...
function parseCardOCR(raw: string): Omit<Transaction,"id"|"confirmed"|"duplicate">[] {
  ...
  for (let i=0; i<lines.length; i++) {
    ...
    results.push({ date:currentDate, time, platform, payCode,
      amount:Math.abs(rawAmt), currency, card, status, source:"card", isDeposit });
  }
  return results;
}
...
function matchTransaction(
  txn:{payCode:string;amount:number;card:string;date:string},
  fbEntries:{payCode:string;amount:number;card:string;date:string}[]
): boolean {
...
function markDuplicates(txns: Transaction[]): Transaction[] {
  ...
  return txns.map(t => {
    if (!t.payCode) return {...t,duplicate:false};
    const key=`${t.payCode.slice(0,6)}|${t.amount.toFixed(1)}|${t.card}`;
    if (seen.has(key)) return {...t,duplicate:true};
    seen.add(key);
...
// ── Report Modal ──────────────────────────────────────────────────────────────
function ReportModal({transactions,onClose}:{transactions:Transaction[];onClose:()=>void}) {
...
<td style="font-family:monospace">${t.payCode?`*${t.payCode}`:"—"}</td>
...
export default function BillingTrackerPage() {
  ...
  useEffect(()=>{
    try { const s=localStorage.getItem("billing_txns3"); if(s) setTransactions(JSON.parse(s)); } catch {}
  },[]);
  ...
  function addManual() {
    const t:Transaction={id:Date.now().toString(),date:"",time:"",platform:"FACEBK",
      payCode:"",amount:0,currency:"USD",card:"",status:"Cleared",confirmed:"Not Yet",
      duplicate:false,source:"manual",isDeposit:false};
    setTransactions(prev=>markDuplicates([t,...prev]));
  }

  return (
    <div className="min-h-screen bg-white">
      {showReport&&<ReportModal transactions={transactions} onClose={()=>setShowReport(false)}/>}
      ...
      <div className="px-6 pt-6">
        ...
        <div className="flex items-start justify-between mb-5">
          ...
          <div className="flex items-center gap-2 flex-wrap justify-end">
            ...
            <button onClick={addManual}
              className="px-3 py-2 border border-gray-200 hover:bg-gray-50 text-gray-600 text-sm font-medium rounded-xl transition-colors">
              + يدوي
            </button>
            <button onClick={()=>cardFileRef.current?.click()} disabled={scanning}
              ...
              {scanning
                ?<><div className="w-3.5 h-3.5 border-2 border-white border-t-transparent rounded-full animate-spin"/>جاري...</>
                :<><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <button onClick={()=>fbFileRef.current?.click()}
              className={`flex items-center gap-2 px-4 py-2 text-sm font-medium rounded-xl transition-colors border ${
                fbLoaded?"bg-blue-50 border-blue-200 text-blue-700":"bg-white border-gray-200 text-gray-600 hover:bg-gray-50"
              }`}>
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              🔵 Facebook PDF / HTML
              {fbLoaded
                ? <span className="text-xs bg-blue-100 text-blue-600 px-1.5 py-0.5 rounded-full">✓ {fbEntries.length} codes</span>
        ...
        {scanMsg&&(
          <div className={`mb-4 px-4 py-2.5 rounded-xl text-sm font-medium ${
            scanMsg.startsWith("✓")?"bg-green-50 text-green-700 border border-green-100":
            scanMsg.startsWith("⚠")?"bg-amber-50 text-amber-700 border border-amber-100":
            scanMsg.startsWith("❌")?"bg-red-50 text-red-700 border border-red-100":
        ...
        <div className="flex items-center gap-3 mb-4 p-3 bg-gray-50 rounded-xl border border-gray-100 flex-wrap">
          ...
          <div className="flex items-center gap-2">
            ...
            <input type="date" value={dateFrom} onChange={e=>setDateFrom(e.target.value)}
              className="text-xs border border-gray-200 rounded-lg px-2 py-1.5 text-gray-700 focus:outline-none focus:border-purple-400"/>
          </div>
          <div className="flex items-center gap-2">
          ...
          <div className="flex gap-1">
            {(["اليوم","أمس","7 أيام"] as const).map((label,i)=>(
              <button key={label} onClick={()=>{
                ...
                const from=new Date(d); if(i===2) from.setDate(from.getDate()-6);
                setDateTo(formatDateInput(d)); setDateFrom(formatDateInput(from));
              }} className="px-2.5 py-1 text-[11px] font-medium bg-white border border-gray-200 rounded-lg text-gray-500 hover:border-purple-300 hover:text-purple-600 transition-all">
                {label}
              </button>
            ))}
          </div>
          {dupCount>0&&(
            <button onClick={()=>setShowDupes(!showDupes)}
              className={`ml-auto px-3 py-1 text-[11px] font-medium rounded-lg border transition-all ${
                showDupes?"bg-orange-50 border-orange-200 text-orange-600":"bg-white border-gray-200 text-gray-400"
              }`}>
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between mb-3">
            ...
            <div className="flex gap-1 bg-gray-100 rounded-xl p-1">
              ...
              <button onClick={()=>setStatusFilter("all")}
                className={`px-3 py-1 rounded-lg text-xs font-medium transition-all ${
                  statusFilter==="all"?"bg-white text-gray-900 shadow-sm":"text-gray-400 hover:text-gray-600"
                }`}>
              ...
              <button onClick={()=>setStatusFilter("OK")}
                className={`px-3 py-1 rounded-lg text-xs font-medium transition-all ${
                  statusFilter==="OK"?"bg-white text-gray-900 shadow-sm":"text-gray-400 hover:text-gray-600"
                }`}>
                ✅ OK ({spendTxns.filter(t=>t.confirmed==="OK").length})
              </button>
              {/* Not Yet */}
              <button onClick={()=>setStatusFilter("Not Yet")}
                className={`px-3 py-1 rounded-lg text-xs font-medium transition-all ${
                  statusFilter==="Not Yet"?"bg-white text-gray-900 shadow-sm":"text-gray-400 hover:text-gray-600"
                }`}>
                ⏳ Not Yet ({notYetCount})
              </button>
              {/* Declined with badge */}
              <button onClick={()=>setStatusFilter("Declined")}
                className={`relative flex items-center gap-1.5 px-3 py-1 rounded-lg text-xs font-medium transition-all ${
                  statusFilter==="Declined"?"bg-white text-gray-900 shadow-sm":"text-gray-400 hover:text-gray-600"
                }`}>
                ❌ Declined
                {declinedCount>0&&(
                  <span className={`inline-flex items-center justify-center min-w-[18px] h-[18px] px-1 rounded-full text-[10px] font-bold transition-all ${
            ...
            {transactions.length>0&&(
              <button onClick={()=>{if(confirm("مسح كل البيانات؟"))setTransactions([]);}}
                className="text-xs text-gray-400 hover:text-red-500 transition-colors">مسح الكل</button>
            )}
          </div>
              ...
              <tbody>
                {filtered.length===0?(
                  <tr><td colSpan={8} className="text-center py-16 text-gray-300">
                    <p className="text-sm">ارفع صور الكارت للبدء</p>
                  </td></tr>
                ):filtered.map(txn=>{
                  const info=PLATFORM_MAP[txn.platform]??PLATFORM_MAP["OTHER"];
                  return (
                    <tr key={txn.id} className={`border-b border-gray-50 transition-colors ${
                      txn.duplicate?"bg-orange-50/60 hover:bg-orange-50":
                      txn.status==="Declined"?"bg-red-50/40":
                      txn.isDeposit?"bg-emerald-50/40":
                      "hover:bg-gray-50/50"
                    }`}>
                      ...
                      <td className="px-3 py-3">
                        <span className={`inline-flex items-center px-2 py-0.5 rounded-lg text-xs font-medium ${info.bg} ${info.color}`}>
                          {info.label}
                        </span>
                      </td>
                      <td className="px-3 py-3">
                        <span className="text-xs font-mono font-semibold text-gray-700">
                          {txn.payCode?`*${txn.payCode}`:"—"}
                        </span>
                      </td>
                      <td className="px-3 py-3">
                      ...
                      <td className="px-3 py-3 text-right">
                        <span className={`text-sm font-semibold ${
                          txn.isDeposit?"text-emerald-600":
                          txn.status==="Declined"?"text-red-400 line-through":
                          "text-gray-800"
                        }`}>
                      ...
                      <td className="px-3 py-3 text-center">
                        {txn.isDeposit?(
                          <span className="inline-flex items-center px-2 py-0.5 rounded-lg text-xs font-medium bg-emerald-50 text-emerald-600">↑ Deposit</span>
                        ):(
                          <span className={`inline-flex items-center px-2 py-0.5 rounded-lg text-xs font-medium ${
                            ...
                            txn.status==="Declined"?"bg-red-50 text-red-500":
                      ...
                      <td className="px-3 py-3 text-center">
                        {txn.isDeposit||txn.status==="Declined"?(
                          <span className="text-xs text-gray-300">—</span>
                        ):(
                          <select value={txn.confirmed}
                            onChange={e=>updateTxn(txn.id,"confirmed",e.target.value as TxConfirmed)}
                            className={`text-xs font-semibold rounded-lg px-2 py-1 border focus:outline-none cursor-pointer transition-all ${
                              txn.confirmed==="OK"
